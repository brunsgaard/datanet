...
\begin{description}
    ...
    \item[Part 2:] Ignoring the overhead of TCP packet headers it would take a
        ...
        milliseconds}$ to fully utilize the connection. %TODO: Calculate in
        another way?
...
\begin{description}
    \item[Part 1:] TCP uses end-to-end, congestion control rather than network
        assisted congetion control, since the IP layer provides no explicit
        feedback to the end systems regarding network
        conngestion.\footnote{K\&R, 5. edition p. 307 }

    \item[Part 2:] When a TCP connection starts up, the Slow Start algorithm
        initializes a congestion window to one segment, after the first
        successful transmission and acknowledgement of a TCP segment the window
        is increased to two segments.

        ``After successful transmission of these two segments and
        acknowledgements completes, the window is increased to four segments.
        Then eight segments, then sixteen segments and so on, doubling from
        there on out up to the maximum window size advertised by the receiver
        or until congestion finally does occur.'' \footnote{\url0}
